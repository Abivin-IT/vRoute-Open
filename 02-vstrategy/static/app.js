(function () {
  ...
  async function getTree(planId) {
    return fetchJson(`/plans/${planId}/tree`);
  }
  async function getScorecard(planId) {
    return fetchJson(`/plans/${planId}/scorecard`);
  }
  async function getSopValidation(planId) {
    return fetchJson(`/plans/${planId}/sop/validate`);
  }
  async function getSignals(planId) {
  ...
  function tryAlert(metadataJson) {
  ...
  function renderPlanInfo(plan) {
    ...
    const mece = Array.isArray(plan.mece_options_json)
      ? plan.mece_options_json
      : tryParse(plan.mece_options_json);
  ...
  async function loadDashboard(planId) {
    ...
    try {
      ...
      app.innerHTML =
        "<!-- Master Data -->" +
        '<div class="vr-section-hdr" id="section-plans">📋 Master Data</div>' +
        '<div class="grid">' +
        '<div class="card full">' +
        renderPlanInfo(plan) +
        "</div></div>" +
        "<!-- Transactions -->" +
        '<div class="vr-section-hdr" id="section-alignment">📝 Transactions</div>' +
        '<div class="grid">' +
        '<div class="card full">' +
        renderTree(treeData.nodes) +
        "</div></div>" +
        "<!-- Reporting -->" +
        '<div class="vr-section-hdr" id="section-scorecard">📊 Reporting</div>' +
        '<div class="grid">' +
        '<div class="card full">' +
        ...
        "</div></div>" +
        "<!-- Configurations -->" +
        '<div class="vr-section-hdr" id="section-mece">⚙️ Configurations</div>' +
        '<div class="grid">' +
        '<div class="card full" id="card-mece-config"></div>' +
        "</div>";
    } catch (e) {
