...
function tryAlert(metadataJson) {
    if (!metadataJson)
        return "";
    try {
        const o = typeof metadataJson === "string"
...
function renderNode(n) {
    ...
    if (n.owner)
        h += `<span class="owner-tag">${n.owner}</span>`;
    h += `<span class="pct">${dot} ${n.progress_pct}%</span></div>`;
    if (alert)
        h += `<span class="alert-tag">\u26A0\uFE0F ${alert}</span>`;
...
function renderPlanInfo(plan) {
    ...
    let h = '<h2>\u{1F4CB} Plan Details & MECE Options</h2><div class="plan-info">';
